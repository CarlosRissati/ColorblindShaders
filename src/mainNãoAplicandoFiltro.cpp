// SOLUÇÃO FINAL: Overlay que NUNCA perde foco e NUNCA para de renderizar
#include <iostream>
#include <glad/glad.h>
...
// ==================== WINDOW PROCEDURE PERSONALIZADO ====================
LRESULT CALLBACK OverlayWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        case WM_PAINT:
            // CRITICAL: Não validar área para forçar repaint contínuo
            // Não chamar ValidateRect - isso força Windows a continuar pintando
            return 0;
            
        case WM_ERASEBKGND:
            // Retornar 1 sem apagar fundo
            return 1;
        ...
        case WM_ACTIVATE:
            // CRITICAL: Sempre retornar que estamos ativos
            return 0;
            
        case WM_SETFOCUS:
        case WM_KILLFOCUS:
            // CRITICAL: Ignorar mudanças de foco
            return 0;
    }
...
public:
    FinalOverlayFilter() : correctionEnabled(true), correctionStrength(0.6f), useLUT(true) {
        std::cout << correctionEnabled << std::endl;
    }
    ...
    bool initialize() {
        ...
        // CRITICAL: Definir como WS_DISABLED para nunca receber foco
        LONG_PTR style = GetWindowLong(overlayHwnd, GWL_STYLE);
        SetWindowLong(overlayHwnd, GWL_STYLE, style | WS_DISABLED);
        
        SetLayeredWindowAttributes(overlayHwnd, RGB(0, 0, 0), 0, LWA_ALPHA);
        
        // CRITICAL: Substituir Window Procedure
        originalWndProc = (WNDPROC)SetWindowLongPtr(overlayHwnd, GWLP_WNDPROC, (LONG_PTR)OverlayWndProc);
        
        // CRITICAL: Forçar janela a continuar renderizando sempre
        SetTimer(overlayHwnd, 1, 16, NULL); // Timer force repaint
        ...
        // CRITICAL: SwapInterval 0 para não depender de VSync
        glfwSwapInterval(0);
        ...
        std::cout << "\n╔════════════════════════════════════════╗" << std::endl;
        std::cout << "║ ✅ OVERLAY INDEPENDENTE DE FOCO       ║" << std::endl;
        std::cout << "╚════════════════════════════════════════╝" << std::endl;
        std::cout << "Agora NUNCA vai travar ao clicar!" << std::endl;
        std::cout << "\nControles:" << std::endl;
    ...
    void run() {
        ...
        std::cout << (correctionEnabled ? "ativado" : "desativado") << std::endl;
        
        while (!glfwWindowShouldClose(window)) {
            // CRITICAL: Poll events sem bloquear
            glfwPollEvents();
            ...
            // Debug: Mostrar FPS a cada 5 segundos
            auto now = steady_clock::now();
            ...
            if (elapsed >= 5) {
                ...
                std::cout << "Capture: " << ((captureFrames - lastFrameCount) / 5) << " FPS" << std::endl;
                lastFrameCount = captureFrames;
                renderFrames = 0;
                lastFpsCheck = now;
            }
            
            // CRITICAL: Não usar Sleep - deixar rodar livre
            // Isso garante que sempre responde imediatamente
        }
    }
    
    void cleanup() {
        KillTimer(overlayHwnd, 1);
        capture->stop();
        delete capture;
        delete shader;
        delete lutLoader;
    ...
    static void keyCallback(GLFWwindow* window, int key, int scancode, int action, int mods) {
        FinalOverlayFilter* filter = (FinalOverlayFilter*)glfwGetWindowUserPointer(window);
        
        if (action == GLFW_PRESS) {
            if (mods == (GLFW_MOD_CONTROL | GLFW_MOD_SHIFT) && key == GLFW_KEY_D) {
                bool current = filter->correctionEnabled.load();
                filter->correctionEnabled.store(!current);
                std::cout << "Filtro " << (!current ? "✅ ATIVADO" : "❌ DESATIVADO") << std::endl;
            }
            else if (mods == (GLFW_MOD_CONTROL | GLFW_MOD_SHIFT) && key == GLFW_KEY_EQUAL) {
                float current = filter->correctionStrength.load();
                filter->correctionStrength.store(std::min(1.0f, current + 0.1f));
                std::cout << "Intensidade: " << (int)(filter->correctionStrength.load() * 100) << "%" << std::endl;
            }
            else if (mods == (GLFW_MOD_CONTROL | GLFW_MOD_SHIFT) && key == GLFW_KEY_MINUS) {
                float current = filter->correctionStrength.load();
                filter->correctionStrength.store(std::max(0.0f, current - 0.1f));
                std::cout << "Intensidade: " << (int)(filter->correctionStrength.load() * 100) << "%" << std::endl;
            }
            else if (mods == (GLFW_MOD_CONTROL | GLFW_MOD_SHIFT) && key == GLFW_KEY_L) {
                if (filter->lutLoader->getIsLoaded()) {
                    bool current = filter->useLUT.load();
                    filter->useLUT.store(!current);
...
int main() {
    std::cout << "\n╔════════════════════════════════════════╗" << std::endl;
    std::cout << "║  Filtro Daltonismo - FINAL v3.0       ║" << std::endl;
    std::cout << "║  100% Independente de Foco            ║" << std::endl;
    std::cout << "╚════════════════════════════════════════╝\n" << std::endl;
    ...
    std::cout << "✅ PRONTO! Clique em QUALQUER LUGAR!" << std::endl;
    std::cout << "   Overlay NUNCA vai parar de renderizar.\n" << std::endl;
